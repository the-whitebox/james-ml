...
def infer(image: np.ndarray, inferencer: Inferencer) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Inference function, return anomaly map, score, heat map, prediction mask and visualisation.
...
if __name__ == "__main__":
    ...
    interface = gr.Interface(
        ...
        outputs=[
            Image(type="numpy", label="Predicted Heat Map"),
            Image(type="numpy", label="Predicted Mask"),
            Image(type="numpy", label="Classification Result"),
        ],
    ...
    interface.launch()
    # return share_link
    # print(f"Shareable link for inference: {share_link}")
